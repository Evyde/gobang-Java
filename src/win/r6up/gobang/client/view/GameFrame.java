...
import win.r6up.gobang.client.MainClass;
import win.r6up.gobang.client.controller.ConfigFileRW;
import win.r6up.gobang.client.module.MethodOfGame;
import win.r6up.gobang.client.module.User;
import win.r6up.gobang.client.module.exception.GameException;
...
public class GameFrame{
	/*
	 * 文本框获得焦点清空其内容类 实现FocusListener接口 无法使用Lambda表达式，故采取内部类方式
	 */
	...
	/*
	 * 游戏窗口 绘制一个棋盘，还要有棋子 当然游戏窗口要重写repaint方法，所以
	 */
	...
	/*
	 * 大厅窗口 绘制一些桌子，桌子数从服务器读取 还有座位，每个桌子两个座位
	 */
	...
	public JFrame creatLoginFrame() throws Throwable {
		...
		registerB.addActionListener(e -> {
			...
			u.setPassword(text2.getPassword().toString());
			try {
				if (MethodOfGame.getmog().getServer().isUserExist(u.name)) {
					try {
					...
					JOptionPane.showMessageDialog(jf, "用户名非法", "错误", JOptionPane.ERROR_MESSAGE);
			} catch (Exception e1) {
